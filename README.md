...
> ⚠ **Note for reviewer.** В рабочем процессе каждая часть ТЗ разрабатывалась в своей `feature/part-N-*` ветке; артефакты работы можно найти там.
...
- ≥8 GB RAM для обычной работы; для cold rebuild `stg_events`, на меньшем cold-rebuild `stg_events` не помещается в один CTAS).

```bash
git clone https://github.com/justkasper/ta-highcore
cd ta-highcore
...
# для запуска из контейнера документацию нужно смотреть как: 
...
После `make build` в `data/warehouse.duckdb` лежат все 11 моделей
...
| Часть                      | Что                                                                                                                                                  | Где лежит                                                                                                                                                                                                                                                          |
| -------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1. Разведка данных         | Полный EDA + free-form ответы                                                                                                                        | [`docs/data_exploration.md`](docs/data_exploration.md) - free-form; подробное исследование оформлено как doc-блок к источнику `raw.events` ([`models/staging/_raw_events__eda.md`](models/staging/_raw_events__eda.md), рендерится в `dbt docs` через `make docs`) |
| 2. Продуктовая постановка  | Декомпозиция продуктовой задачи и решения                                                                                                            | [`docs/dashboard_sketch.md`](docs/dashboard_sketch.md) - фактура решения, допущения собраны в этом файле.                                                                                                                                                          |
| 3. dbt-проект              | 11 моделей в 5 слоях (staging -> int -> utils -> marts/core -> marts/reports), ~70 generic + 8 singular + 11 unit ≈ 90 тестов; tier-фрейм и принципы | `models/`, `tests/`, [`docs/decisions/architecture.md`](docs/decisions/architecture.md) - основная архитектура и принципы, <br>[`docs/decisions/testing-strategy.md`](docs/decisions/testing-strategy.md) - принципы для написания тестов                          |
...
| 5. Кастомный скилл         | `dbt-docs`: преобразовывает (собирает) документацию в формат человек + агент; минимум инициативы, в спорных ситуациях проставляет tag: check         | [`skills/dbt-docs/`](skills/dbt-docs/)                                                                                                                                                                                                                             |
...
Дашборд читает только `marts/reports/*`: никакой бизнес-логики на стороне BI.
...
> ⚠ **Public-sample artefact:** ровно **50 000 событий/день, 114 дней** (5.7M строк). Cohort sizes и абсолютные числа не репрезентативны — читать только относительно. Все принятые ниже решения «висят» на этом фрейминге.
...
| 5   | `profiles.yml` зажат под 8 GB машины (`threads=1`, `memory_limit=1500MB`)                                | На проде стандартный thread-pool BQ (irrelevant)                                                                                                             | Без капов cold rebuild SIGKILL'ится на 8 GB. Цена: builds на больших машинах работают медленнее, чем могли бы. Acceptable trade-off для воспроизводимости (см. коммит `9b091db`).                                                                        |
| 6   | Dedup в `stg_events`: hash-aggregate (`list_agg(_src_rowid)[2:]` + anti-join), не `qualify row_number()` | На BQ обратно `qualify` (естественно ложится)                                                                                                                | DuckDB `qualify` требует глобального sort'а на 5.7M; hash-aggregate per-group sort внутри `having count(*) > 1` (99% групп выкидываются). Memory footprint меньше, миграция на BQ через `qualify` чистая. Сам OOM это не лечит, bottleneck в write-фазе. |
| 7   | Density тестов: **core толще, reports тоньше** (~12 vs ~7 на модель)                                     | То же, намеренная конфигурация, не trade-off                                                                                                                 | Изначально было наоборот (30 тестов на 4 mart'а vs 13 на core). Reverted после tier-аудита: source of truth должен быть толще, а на reports `not_null` поверх `coalesce(..., 0)` low-signal.                                                             |
...
1. `firebase_export_to_warehouse` (внешний контракт; владелец: data-platform)
...
2. `dbt_models_daily`: собственно трансформации.
...
- Альтернатива — **Astronomer Cosmos**: парсит dbt-проект и автоматически разворачивает модели в нативные Airflow-таски с зависимостями из manifest'а; три task-group'ы делаются через `select` по тегам, параллелизм mart'ов — из коробки. Минусы: переключение incremental ↔ full-refresh нужно решать через `operator_args` или отдельные DAG'и, manifest стоит грузить в режиме `LoadMode.DBT_MANIFEST` (иначе медленный DAG-parse), freshness-цепочку проще оставить вне Cosmos.

3. `dbt_freshness_and_observability` (отдельный DAG)

- Расписание: раз в сутки, сразу после `firebase_export_to_warehouse`.
...
- Дополнительно — алерт на отсутствие sentinel'а к ожидаемому времени: ловит случаи, когда Firebase export опоздал или упал. Это основной сигнал «данные не приехали»; `source freshness` — вторичная проверка на случай, когда sentinel записан, но `loaded_at_field` отстаёт.
### 5.2. Расписание

- `firebase_export_to_warehouse`: суточно, ~05:30 UTC (под Firebase SLA).
- `dbt_freshness_and_observability`: триггерится сенсором на sentinel из (1), стартует перед `dbt_models_daily` как gate.
- `dbt_models_daily`: триггерится сенсором на sentinel из (1) после успешного `dbt_freshness_and_observability`.

### 5.3. Зависимости и selectors

- Внутри `dbt_models_daily`: явные task-зависимости через TaskGroup'ы (`staging_intermediate` >> `marts_core` >> `marts_reports`), внутри groups параллельность определяется dbt-графом из manifest'а.
- На уровне dbt: `+stg_events` для upstream, `marts.core+` для downstream-impact, `state:modified+` в CI для PR-prune'ов.
- Backfill: запускается через отдельный DAG `dbt_backfill_manual` (см. 5.4) с параметрами `cohort_date_from` / `cohort_date_to`; передаются в dbt `--vars '{backfill_from: ..., backfill_to: ...}'`, инкрементальные модели читают эти vars в `where` условиях фильтра партиций.

### 5.4. Обработка сбоев

- **Source freshness gate** (отдельный DAG `dbt_freshness_and_observability` перед `dbt_models_daily`): если `dbt source freshness` возвращает `error` (партиция D отсутствует в `raw.events`), DAG fail'ится с ясной диагностикой, `dbt_models_daily` не стартует, пустые mart'ы не собираются. На статичном публичном сэмпле этот gate отключён (zero-day SLA не имеет смысла на 2018-09 данных).
- **Tier 1 тесты** (PK/FK) = `severity: error` -> fail таска -> downstream task'и в группе помечаются `upstream_failed`, PagerDuty wakes oncall.
...
- `dbt run` пишет `target/run_results.json` и `manifest.json` -> загружается post-processing task'ом в конце `dbt_models_daily` в metadata-таблицу (`dbt_metadata.runs` / `.tests` / `.timing`).
- Поверх metadata-таблицы Grafana-дашборд с ключевыми метриками: per-model duration trend, test pass rate, freshness lag.
- Важные failures (Tier 1) -> Slack `#data-alerts` + PagerDuty; warns -> `#dbt-warns`; daily summary -> Slack `#data-digest` в 09:00 UTC.
...
8. **Latency / SLA**: суточное обновление окей, или нужен near-real-time?
...
├── CLAUDE.md                       # project conventions для Claude Code
├── Makefile                        # setup / build / build-small / docs / clean
├── requirements.txt                # зафиксированные зависимости
├── packages.yml                    # dbt_utils
├── package-lock.yml                # dbt-пакеты, зафиксированные версии
...
│
├── macros/
│   ├── _macros.yml                 # документация макросов (single source)
│   ├── cum_sum.sql                 # cumulative window helper
│   ├── revenue_metrics_columns.sql # shared SELECT-list для revenue mart'ов
│   └── trailing_avg.sql            # trailing 4w baseline
│
├── tests/                          # singular business-checks
│   ├── _tests.yml                  # документация singular-тестов (single source)
...
│   ├── assert_n_purchases_reconciliation.sql
│   ├── assert_paying_users_reconciliation.sql
│   ├── assert_revenue_days_are_activity_days.sql
...
- **«Хочу пощупать данные руками»**: `make build`, затем `make docs` (интерактивный dbt docs на `localhost:8080`) или прямой коннект к `data/warehouse.duckdb` через `duckdb` CLI.

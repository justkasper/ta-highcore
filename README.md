...
Стек: **DuckDB + dbt-duckdb** локально (как замена BigQuery), Python 3.11+ для подготовки данных, matplotlib для рендера мокапов дашборда. На проде архитектура без переписывания SQL ложится на BigQuery, отличается только конфиг материализаций (см. [`docs/architecture.md`](docs/architecture.md) §«Будущая миграция в BigQuery»).
...
>
> - `feature/part-1-data-exploration`: `docs/data_exploration.md`, `docs/eda_report.md`, `docs/eda_tests.md`, `duckdb-explore.sh`
> - `feature/part-2-product-framing`: `docs/assumptions.md`
> - `feature/part-3-architecture-doc`: `models/`, `tests/`, `docs/architecture.md`, `docs/testing_plan.md`
...
| Часть | Что | Где лежит |
|---|---|---|
| 1. Разведка данных | Полный EDA + free-form ответы; каталог 13 аномалий, identifier model, монетизация | [`docs/data_exploration.md`](docs/data_exploration.md), [`docs/eda_report.md`](docs/eda_report.md) |
| 2. Продуктовая постановка | 16 фиксированных решений (identity, cohort anchor, окно D30, definition of active, slices) с обоснованиями и карта «допущение -> витрина» | [`docs/assumptions.md`](docs/assumptions.md) |
| 3. dbt-проект | 11 моделей в 5 слоях (staging -> int -> utils -> marts/core -> marts/reports), ~70 generic + 8 singular + 11 unit ≈ 90 тестов; каталог инвариантов и tier-фрейм | `models/`, `tests/`, [`docs/architecture.md`](docs/architecture.md), [`docs/testing_plan.md`](docs/testing_plan.md), [`docs/eda_tests.md`](docs/eda_tests.md) |
...
Полное обоснование (9 ключевых архитектурных решений с альтернативами, которые отвергли) в [`docs/architecture.md`](docs/architecture.md) §«Ключевые архитектурные решения».
...
Полный план покрытия с картой gap'ов и принципами отбора в [`docs/testing_plan.md`](docs/testing_plan.md).

---

## 3. Допущения и уточнения

Полная таблица из 16 решений (с одной строкой обоснования каждое и ссылками на конкретные числа EDA) в [`docs/assumptions.md`](docs/assumptions.md) §2. Самое важное здесь:
...
Полная аннотация в [`docs/architecture.md`](docs/architecture.md) §«Что мы намеренно НЕ строим».
...
- **Drift-watchers** (`severity: warn`, см. `testing_plan.md` §4.8): только лог, без alert'а; цель наблюдать тренд через Grafana.
...
Решения, что зафиксированы в [`docs/assumptions.md`](docs/assumptions.md) §3, выбраны под публичный 2018-сэмпл, на проде их стоит верифицировать.
...
8. **BI-инструмент**: Metabase / Superset / Looker / Tableau? Влияет на форму mart'ов (long-form vs wide-form), на metadata-import'ы, на тип фильтров (single vs multi-select).
9. **Latency / SLA**: суточное обновление окей, или нужен near-real-time? Сейчас ориентир на ежесуточный refresh после Firebase-export SLA. NRT потребует переезда на Streaming inserts + materialized views в BQ.
10. **Ownership витрин**: кто owner за `mart_*` таблицы? AE-команда, или передаём в analytics-team как stable interface? От этого зависит SLA на breaking changes.
11. **Backfill policy**: как часто перестраиваем исторические партиции (sparse `fct_user_daily` это упрощает, но не отменяет)? И с какого момента в прошлом делаем полную пересборку при изменении бизнес-логики (например, нового определения active)?
12. **Cost guardrails на BQ**: при on-demand pricing какой quota cap на день / месяц? Sparse + partitioned mart'ы держат типичный dashboard-query в пределах одной партиции (~5-10 MB), но full-refresh `mart_revenue_overall` поверх 5 лет данных другая история.
...
├── models/
│   ├── staging/
│   │   ├── _sources.yml            # source-контракт raw.events
│   │   ├── _models.yml
│   │   └── stg_events.sql          [view]
│   ├── intermediate/
...
│   ├── eda_report.md               # Part 1: free-form ответ ТЗ
│   ├── eda_tests.md                # Part 1: каталог инвариантов
│   ├── assumptions.md              # Part 2: 16 решений + вопросы
│   ├── architecture.md             # Part 3: слои, materializations, BQ-mig
│   ├── testing_plan.md             # Part 3: tier-фрейм, density карта
...
- **«Я ревьюер, у меня 10 минут»**: этот README + быстрый просмотр [`docs/architecture.md`](docs/architecture.md) §TL;DR + §«Карта моделей».
- **«Хочу понять, что в данных»**: [`docs/data_exploration.md`](docs/data_exploration.md).
- **«Хочу понять, что считаем и почему именно так»**: [`docs/assumptions.md`](docs/assumptions.md).
- **«Хочу понять архитектуру и материализации»**: [`docs/architecture.md`](docs/architecture.md).
- **«Хочу понять, как тестируется»**: [`docs/testing_plan.md`](docs/testing_plan.md).

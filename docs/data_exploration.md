...
**События** — 37 различных `event_name`. Грейн: одна строка на одно эмитированное Firebase-событие. Топ по объёму (§Event vocabulary):
...
- **Кодировки/типы шумят**: смешанные value-типы в `event_params` (`level`), литерал `'NaN'` в `device.operating_system`, перезапись `uft, требует нормализации в staging.

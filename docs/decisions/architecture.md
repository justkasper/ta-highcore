...
Какими решениями обусловлена структура слоёв, грейн фактов и материализации. Колонки моделей, типы и описания живут в `_models.yml` и рендерятся в `dbt docs` — этот файл их не повторяет, а фиксирует **почему** архитектура именно такая.
